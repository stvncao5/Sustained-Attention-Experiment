function [memData] = SustAttnMemTest(varargin)
%
...
% main parameters
nTrials = 200; % default is 200; having too many more trials than were in the previous task will probably crash the program
...
% i'm honestly not sure why 100 is being used as a magic number here, but this effectively means that attCategs has a length of 200 values
memData.trial = 1:nTrials;
memData.attCategs = Shuffle([prepotentCateg*ones(1,100) counterCateg*ones(1,100)]);

% within each type of trial, make half of them correspond to previously shown images, and half of them to new images
...
memData.memCategs(memData.attCategs==counterCateg)=Shuffle([counterFamiliarTrials*ones(1,50) counterForeignTrials*ones(1,50)]);
